---
title: "Covid-19 SIR Modeling Kelantan"
author: "Wan Nor Arifin (USM)"
...
## Description
This SIR model is modeled by finding the best parameters that fit to Covid-19 Kelantan data up to `r format(Sys.time(), '%a, %e %b %Y')`.
...
```{r, message=FALSE}
mys_data = read_csv("covid-19_kel.csv") %>% rename(Date = "date")
# mys_data = read_csv("https://raw.githubusercontent.com/wnarifin/covid-19-kelantan/main/covid-19_kel.csv") %>% rename(Date = "date")
...
parameters_values = c(1/2, 1/14)
# parameters_values = c(0.273, 0.033)
parameters_values_lower = c(0.1, 0.01)
# parameters_values_upper = c(0.35, 0.07)  # max gamma 0.7 i.e. 14 days
parameters_values_upper = c(0.5, 0.1)  # allows freedom in parameter space exploration
...
# Initial values
N           = 1.9E6/2  # Jab Perangkaan M'sia 2020
...
fitplot1 = ggplot(fitted_projected, aes(x = Date)) + geom_line(aes(y = I, color = "Infectious")) + 
  ...
  labs(y = "Number of cases", title = paste("COVID-19 SIR model Kelantan, fitted and observed", name),
...
fitplot1_log = ggplot(fitted_projected, aes(x = Date)) + geom_line(aes(y = I, color = "Infectious")) + 
  ...
  labs(y = "Number of cases", title = paste("COVID-19 SIR model Kelantan, fitted and observed,", name, "log10"),
       subtitle = paste("Projection from data:", start_date, "to", fitted_projected$Date[max(Day)]),
       color = paste0("Model fit:\n",
                      "R square (I) = ", round(R2_1,3), "\n",
                      "R square (R) = ", round(R2_2,3), "\n",
...
sirplot1 = ggplot(fitted_projected, aes(x = Date)) + 
  ...
  scale_colour_manual(values = colors) +
  labs(y = "Number of cases", title = paste("COVID-19 SIR model Kelantan,", name), 
       subtitle = paste("Projection from data:", start_date, "to", fitted_projected$Date[max(Day)]),
       color = paste0("Model fit:\n",
                      "R square (I) = ", round(R2_1,3), "\n",
                      "R square (R) = ", round(R2_2,3), "\n",
...
sirplot1_log = ggplot(fitted_projected, aes(x = Date)) + 
  ...
  scale_x_date(date_breaks = "14 day", date_labels = "%d/%m/%y") + 
  scale_colour_manual(values = colors) +
  labs(y = "Number of cases", title = paste("COVID-19 SIR model Kelantan,", name, "log10"), 
       subtitle = paste("Projection from data:", start_date, "to", fitted_projected$Date[max(Day)]),
       color = paste0("Model fit:\n",
                      "R square (I) = ", round(R2_1,3), "\n",
                      "R square (R) = ", round(R2_2,3), "\n",
